...
export const getStudentsByClass = async (req, res) => {
  try {
    ...
    if (!className) {
      return res.status(400).json({ success: false, message: 'Please provide a class' });
    }
...
export const createAttendance = async (req, res) => {
  try {
    ...
    if (!className || !date || !students?.length) {
      return res.status(400).json({ success: false, message: 'Provide class, date, and students array' });
    }

    const invalid = students.some(s => !s.student || !['present', 'absent'].includes(s.status));
    if (invalid) {
      return res.status(400).json({ success: false, message: 'Each student needs ID and valid status' });
    }
    ...
    res.status(201).json({ success: true, data: attendance, message: 'Attendance created' });
  } catch (error) {
...
export const updateAttendance = async (req, res) => {
  try {
    ...
    if (req.body.date || req.body.class) {
      ...
      if (exists) return res.status(400).json({ success: false, message: 'Duplicate attendance' });
    }
...
export const deleteAttendance = async (req, res) => {
  try {
    ...
    if (attendance.createdBy.toString() !== req.user._id.toString() && req.user.role !== 'admin' && req.user.role !== 'staff') {
  ...
  try {
    ...
    const result = records.map((record) => {
      const studentRecord = record.students.find(
        (s) => s.student && s.student._id.toString() === studentId.toString()
      );
      ...
      return {
        ...
        name: studentRecord?.student?.name || 'Unknown',
        admissionNumber: studentRecord?.student?.admissionNumber || 'N/A'
      };
    });
